...
**👀 过程透明,随时插手。** 思考与回复分离呈现 —— 桌面应用实时铺开完整过程(思考流、工具卡片、运行中可改向);企微里实时滚动思考摘要与工具执行流，审批/clarify 弹交互卡片，回复到达时整条替换，像折叠了一样干净。任何时刻 `/stop` 打断;任务运行中发新消息会作为 steer 在下一个迭代边界注入,不打断、只纠偏。

**🖥️ 它干活,你看得见 —— 在真实的终端里。** agent 跑的每条命令都流进按对话划分的实时控制台:长任务跑到一半,终端抽屉自动弹开接着看(秒级命令不会打扰你),常驻进程有自己的标签页;agent 开的 SSH 会话是共享的 —— 你可以直接在同一个会话里打字,角标标明谁在输入。你自己的多标签本地终端也在手边 —— 可交互、与对话无关。

**🛡️ 危险操作有人把关。** 危险命令模式 + 高危参数表 + LLM 语义判定,三端一致的人工确认:桌面审批卡、企微交互按钮卡、聊天回复 `y|n|a`、CLI 提示。`deny`/`allow` glob 规则与会话级「批准且不再询问」让闸门不变成噪音。
