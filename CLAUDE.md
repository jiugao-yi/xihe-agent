...
## Gotchas
- **三模式同步考虑（硬规则）**：所有功能——tool、skill、提示词以及一切新能力——设计时必须同时考虑 **CLI、gateway（企微/飞书）、桌面端（serve）** 三种运行模式的呈现与投递通道。能力可以按模式降级（网关 4000 字限制且无代码块渲染、CLI 无 JS、桌面全量），但必须显式为每种模式给出行为，不能只按桌面端做。模式差异写进 tool description / skill 正文（模型经 `PLATFORM_PROMPTS` 已知当前平台），不靠临场发挥。
...
- **Tools stay decoupled from each other**: a tool module must not import another tool module (directly or via its internals), and must not *name* other tools in hint/描述 text — which image/read tool to apply is the agent's own routing decision. Shared machinery tools need (LLM Describe of images, aux calls, etc.) lives in core (`core/agent/auxiliary_client.py` 的 `describe_image` 是范例) — importing core from tools is always fine, tool↔tool never.
