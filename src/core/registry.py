"""Tool-registry contract: entries, the registry singleton, dispatch, and the
JSON result/error envelope every handler returns.

Lives in core (not tools) because both sides depend on it: the agent loop
dispatches through ``registry``, and tool modules register into it at import
time. ``tools/__init__`` re-exports these names so tool modules keep the
short ``from tools import registry, tool_result, tool_error`` form.

Layering rule: core must not import tools at module level — importing a tool
module registers tools (import side effect) and would let the engine reach
plugins before the composition root (core.context) has loaded them.
Function-level delayed imports are the accepted seam; core.support (approval
gate, path rewriting) is the zero-dependency exception that may be imported
at module level from anywhere.
"""
