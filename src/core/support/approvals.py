...
_DANGEROUS_PATTERNS = [
    ...
    # taskkill /im xihe.exe 杀掉的是宿主进程本身：turn 会随进程一起静默消失
    # （2026-08-27 桌面端会话无声中断的根因）。/PID 形式无法在正则层识别。
    (r'\b(taskkill|stop-process)\b.*\bxihe(\.exe)?\b', "kill agent process (self-termination)"),
...
# /PID 形式的自杀：命令文本里没有 "xihe" 字样，正则表永远看不见，但杀掉的
# 就是运行本判定代码的进程（2026-08-27 桌面端会话无声中断的另一半根因）。
_PID_TARGET_RE = re.compile(
    r'(?:/pid\s+|-id\s+|\bkill\s+(?:-\d+\s+)*)(\d{1,8})', re.IGNORECASE)


def _targets_own_pid(command: str) -> bool:
    own = os.getpid()
    return any(int(m) == own for m in _PID_TARGET_RE.findall(command or ""))
...
_HIGH_RISK = {
    ...
    "computer_app": lambda a: (
        f"结束桌面应用进程：{a.get('name') or '?'}"
        if _action(a) == "close" else None
    ),
...
def _danger_detail(name: str, args: dict) -> tuple[str, str] | None:
    ...
    if name == "terminal":
        ...
        if dangerous:
            return desc, f"危险命令（{desc}）：{command}"
        if _targets_own_pid(command):
            return ("kill agent process (self-termination)",
                    f"危险命令（kill agent process (self-termination)）：{command}")
        return None
...
def _llm_judge_command(command: str, aux: Optional[Any]) -> dict | None:
    ...
    if resp is None:
        return None
    try:
        from core.agent.auxiliary_client import extract_content_or_reasoning
