...
def _reshape_history(rows: list, marks: dict = None) -> list:
    ...
    ``marks`` ({session_id: reason}) labels reset boundaries: when the round
    tag (row session_id) changes mid-stream, the first folded item of the new
    round carries ``round_start: True`` + ``reset_reason`` — the client draws
    a divider there.
    """
    out: list = []
    ...
    marks = marks or {}
    prev_sid = None
    pending = None  # stashed (round_start, reason) awaiting the next emitted item
    ...
    def flush():
        ...
        if cur is not None:
            ...
            rr = cur.get("round_reason")
            if rr is not None:
                item["round_start"] = True
                if rr:
                    item["reset_reason"] = rr
            out.append(item)
            cur = None
    ...
    for m in rows:
        ...
        if sid is not None:
            if prev_sid is not None and sid != prev_sid:
                pending = marks.get(sid, "")  # "" = boundary with no recorded reason
            prev_sid = sid
        role = m.get("role")
        if role == "tool":
        ...
        if role == "user":
            ...
            flush()
            item = {"role": "user", "content": m.get("content") or "",
                    "id": m.get("id"), "tools": 0,
...
class ChatMixin:
    ...
    # 上传根目录：按对话隔离，serve 不会清理（用户自己删，或整个 conv 删除
    # 时随之留在磁盘）。文件名带时间戳防覆盖。
    _UPLOAD_ROOT = None  # resolved lazily — AGENT_HOME import is cheap but explicit

    def _upload_dir(self, conv_id: str) -> Path:
        from core.config import AGENT_HOME
        d = Path(AGENT_HOME) / "uploads" / str(conv_id)
        d.mkdir(parents=True, exist_ok=True)
        return d

    async def upload_attachment(self, request):
        """POST /convs/{conv_id}/upload (multipart) → {"path", "name", "size"}.

        桌面上传附件的唯一入口：文件落 AGENT_HOME/uploads/<conv_id>/，返回
        服务器端绝对路径 + 原始文件名，随 send 帧的 attachments 字段回传。
        大小上限 _MAX_UPLOAD_MB；扩展名不限（安全边界在工具层——读取走
        read_file 的既有规则）。
        """
        ...
        if field is None or field.name != "file":
            return web.json_response(
                {"error": "expected a multipart field named 'file'"}, status=400)
        # 客户端 filename 可能带 URL 编码的分隔符（aiohttp 传 ../ 为
        # ..%2F..%2F），先解码再取 basename，双重防路径穿越。
        import urllib.parse as _up
        name = _up.unquote(field.filename or "attachment")
        name = str(Path(name).name) or "attachment"
        data = bytearray()
        while True:
            ...
            if not chunk:
                break
            data.extend(chunk)
            if len(data) > _MAX_UPLOAD_MB << 20:
                return web.json_response(
                    {"error": f"file exceeds {_MAX_UPLOAD_MB}MB limit"}, status=413)
        import time as _time
        dest = self._upload_dir(conv_id) / \
            f"{_time.strftime('%Y%m%d_%H%M%S')}_{name}"
        dest.write_bytes(bytes(data))
        logger.info("serve upload: conv=%s name=%s size=%d", conv_id, name, len(data))
        return web.json_response(
            {"path": str(dest), "name": name, "size": len(data)})

    def _describe_attachments(self, attachments: list) -> list | None:
        """附件元数据就绪：图片补视觉描述（秒级 LLM 调用——调用方必须放到
        executor 线程跑，不能卡事件循环）。返回持久化用的 attachments 数组；
        无附件返回 None。消息文本不在此拼接：content 保持用户原文，模型
        面向的提示块由 agent 在 API 边界注入（见 _attachment_hint）。"""
        from core.agent.auxiliary_client import describe_image
        out = []
        for a in attachments:
            p = str(a.get("path"))
            item = {"name": str(a.get("name") or Path(p).name),
                    "path": p, "size": a.get("size")}
            if Path(p).suffix.lower() in (".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp"):
                try:
                    item["desc"] = describe_image(self.ctx.aux, p)
                except Exception as e:
                    logger.warning("attachment describe failed for %s: %s", p, e)
            out.append(item)
        return out or None
            ...
            async for msg in ws:
                if msg.type == web.WSMsgType.TEXT:
                    ...
                    if ctype == "send":
                        ...
                        self._clarify(str(cmd.get("conv_id") or ""),
                                      cmd.get("id"),
    ...
    async def _handle_send(self, ws, cmd):
        ...
        # 附件（桌面 /upload 先落盘，这里拿 path 列表注入消息文本）。图片
        # 走视觉自动描述（与 gateway 同一 describe_image_sync），普通文件
        # 只注入路径 + 大小，agent 用 read_file 等工具按需取。
        attachments = [a for a in (cmd.get("attachments") or [])
        ...
        if not str(text).strip() and not attachments:
            ...
            return
        # 图片描述是秒级视觉 LLM 调用——放 executor 线程，不卡事件循环。
        att_meta = None
        if attachments:
            loop = asyncio.get_running_loop()
            att_meta = await loop.run_in_executor(
                None, lambda: self._describe_attachments(attachments))
        ...
        async with self._turn_lock(conv_id):
            # Turn actually running now (not queued) — feeds list_sessions'
            # running flag. Cleared in the finally below.
            self._running_convs.add(conv_id)
            ...
            # `auto_reset` predicts (same policy check get_or_create_session
            # runs moments later) that this send crosses a reset boundary —
            # the client flags its optimistically-appended user bubble with a
            # round divider right away instead of waiting for a refetch.
            if not await self._safe_send(self._conv_sockets.get(conv_id) or ws, {
                "type": "turn_start", "turn_id": turn_id, "conv_id": conv_id,
                "session_key": session_key,
                "auto_reset": self.ctx.db.pending_auto_reset(source),
            }):
            ...
            if cwd and not Path(str(cwd)).is_dir():
                ...
                cwd = None
            # Plan mode (desktop toggle): read-only planning turn; a user
            # approval on the plan's clarify card auto-launches the execution
            # turn.
            plan_requested = bool(cmd.get("plan"))
            ...
            try:
                params = AgentTurnParams(
                    source=source, text=str(text), cwd=cwd,
                    attachments=att_meta,
                    plan=plan_requested,
                    regenerate=bool(cmd.get("regenerate")),
                    approval_key=approval_key)
                runner = AgentTurnRunner(
                    self.ctx, params,
                    TurnCallbacks(
                        stream_delta=emitter.on_delta,
                        tool_call_start=emitter.on_tool_start,
                        tool_result=emitter.on_tool_result,
                        approval_request=emitter.on_approval_request,
                        approval_result=emitter.on_approval_result,
                        clarify_request=emitter.on_clarify_request,
                        clarify_result=emitter.on_clarify_result,
                    ))
                agent = runner.agent
            except Exception as e:
            ...
            def _worker():
                try:
                    result = runner.run()
                    return result.text, result.exit_reason, result.usage, result.error
                except Exception as e:
            ...
            try:
                ...
                if err:
                    ...
                    outcome = runner.plan_outcome()
                    if outcome:
                        saved = f"\n\n📄 计划已保存：{outcome.path}" if outcome.path else ""
                        done["text"] = (outcome.body + saved
                                        + "\n\n▶ 开始按计划执行…")
                        # The execution turn re-enters _handle_send (same
                        # turn lock serializes it after this one): full
                        # tool surface, plan as the contract.
                        exec_cmd = dict(cmd, type="send", plan=False,
                                        text=outcome.exec_text)
                        asyncio.create_task(self._handle_send(ws, exec_cmd))
                    await self._safe_send(target, done)
            finally:
                self._running_convs.discard(conv_id)
    ...
    def _clarify(self, conv_id: str, question_id, answer: str) -> None:
        """Resolve the active turn's pending clarify (desktop card option
        click / free-text submit)."""
        if not conv_id:
            return
        with self._active_lock:
            agent = self._active.get(conv_id)
        if agent:
            try:
                if not agent.resolve_clarification(
                        str(question_id) if question_id else None, answer):
                    logger.info("clarify for conv=%s matched no pending question "
                                "(id=%s)", conv_id, question_id)
            except Exception:
                logger.warning("clarify failed for conv=%s", conv_id, exc_info=True)
        ...
        return web.json_response({"sessions": [
            {
                ...
                "running": r.get("chat_id") in self._running_convs,
            }
            for r in rows
        ]})
    ...
    async def get_messages(self, request):
        ...
        def work():
            source = self._source(conv_id)
            session_key = self.ctx.db.build_key(source)
            # Cross-round transcript: the conversation view keeps pre-reset
            # history; the agent's own context still loads the current round
            # only (load_messages on the live session_id).
            rows = self.ctx.db.load_transcript(session_key)
            marks = self.ctx.db.reset_marks(session_key)
            # A reset whose new round has no messages yet: flag it so the
            # client draws a trailing divider right after the reset, instead
            # of the divider only appearing with the next message.
            pending_reset = None
            entry = self.ctx.db.get_entry(session_key)
            if entry and all(r.get("session_id") != entry.session_id for r in rows):
                pending_reset = marks.get(entry.session_id, "")
            return {"conv_id": conv_id,
                    "messages": _reshape_history(rows, marks),
    ...
    async def get_trace(self, request):
        """Lazy-fetch one turn's tool-call trace by its anchor row id (uuid).
        ...
        """
        conv_id = request.match_info["conv_id"]
        anchor_id = (request.match_info.get("msg_id") or "").strip()
        if not anchor_id:
        ...
        def work():
            ...
            if start is not None:
                ...
                for item in ordered:
                    ...
                    row = results_by_id.get(tcid)
                    if row is not None:
                        # 审批决议在 message_meta（持久化时从 result JSON 剥离）——
                        # 给桌面渲染审批徽章；content 保持纯工具输出。
                        rec = row.get("approval")
                        if isinstance(rec, dict):
                            te["approval"] = rec
                        result = row.get("content")
                        if isinstance(result, str):
                            truncated = len(result) > _WS_RESULT_LIMIT
                            te["result"] = result[:_WS_RESULT_LIMIT] if truncated else result
    ...
    async def truncate_conv(self, request):
        ...
        try:
            ...
            body = {}
        from_id = str((body or {}).get("from_msg_id") or "").strip()
        if not from_id:
        ...
        def work():
            ...
            if not entry:
                return None
            return self.ctx.db.truncate_messages_from(session_key, entry.session_id, from_id)
...
def add_routes(router, app):
    ...
    router.add_post("/convs/{conv_id}/upload", app.upload_attachment)
