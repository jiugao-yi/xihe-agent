...
from core.support.interrupt import interruptible_iter, is_interrupted
...
def _write_file(args: dict, **kw) -> str:
    ...
    try:
        ...
        # Pre-write snapshot for the result diff (same shape as _patch's — the
        # desktop trace renders it). Oversized/binary files skip the diff but
        # still report existence for `created`.
        existed = False
        old_text = None
        try:
            if p.is_file():
                existed = True
                if p.stat().st_size <= 512000:
                    old_text = p.read_text(encoding=encoding)
        except (OSError, UnicodeDecodeError):
            old_text = None

        stale_warning = _check_staleness(str(p))
        # Overwrite destroys whatever landed in the file since our last read
        # (external edit / concurrent agent) — refuse and make the agent
        # re-read, force is the explicit escape hatch. Append only adds, so
        # a warning is enough there.
        if stale_warning and mode != "append" and not args.get("force"):
            return tool_error(
                f"{stale_warning} Overwrite refused — read_file the current "
                "content first, or pass force=true to overwrite anyway.",
                path=str(p),
            )
    ...
    schema={
        ...
        "function": {
            ...
            "description": (
                ...
                "Refused when the file changed since you last read it (read_file again, or force=true). "
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "encoding": {"type": "string", "description": "File encoding (default: utf-8)"},
                    "force": {"type": "boolean",
                              "description": "Overwrite even though the file changed since you last read it "
                                             "(staleness guard). Default false — prefer re-reading and merging."},
                },
