...
def handle_during_turn(text: str, agent, cmd_ctx: Dict[str, Any]):
    ...
    if head in ("/stop", "/cancel") or (not head and is_stop_intent(text)):
        ...
        if try_resolve_clarify(agent, text):
            cprint("[clarify] 回答已送达", Ansi.DIM)
            return
        agent.steer(text)
        preview = text[:60]
...
class _CliEmitter:
    """Terminal-side callback bundle for one turn: buffers streamed text and
    renders it as markdown at flush points; prompts for approval/clarify go
    straight to the console (the REPL routes the user's reply back in)."""

    def __init__(self, on_content=None, emit_func=None):
        self.on_content = on_content
        self.emit_func = emit_func
        self.sbuf = {"text": "", "kind": None}

    def _emit(self, text, **kw):
        cprint(text, **kw)

    def flush_buffer(self):
        if not self.sbuf["text"]:
            return
        if self.on_content and self.sbuf["kind"] != "reasoning":
            self.on_content(self.sbuf["text"], "content")
        else:
            ...
            color = Ansi.DIM if self.sbuf["kind"] == "reasoning" else Ansi.GREEN
            cprint(self.sbuf["text"], color, emit_func=self._emit)
        self.sbuf["text"] = ""

    def stream_delta(self, text: Optional[str], kind="content", by=None):
        if text is None:
            self.flush_buffer()
            return
        if self.sbuf["kind"] is not None and self.sbuf["kind"] != kind:
            self.flush_buffer()
        self.sbuf["kind"] = kind
        if self.on_content:
            self.sbuf["text"] += text
    ...
    def tool_start(self, name: str, args_summary: str, by=None, tool_call_id=None):
        self.flush_buffer()
        short_args = args_summary[:80] + ("..." if len(args_summary) > 80 else "")
        tag = f"[{by}] " if by else ""
        cprint(f"  * {tag}{name}({short_args})", Ansi.DIM, emit_func=self._emit)

    def tool_call(self, name: str, args_summary: str, elapsed: float, by=None):
        self.flush_buffer()
        tag = f"[{by}] " if by else ""
        cprint(f"  OK {tag}{name} ({elapsed:.1f}s)", Ansi.DIM, emit_func=self._emit)

    # 审批回调在工具线程触发（dispatch 内）；提示走 cprint 直出。
    # 批复本身经主线程 REPL 的 handle_during_turn → try_resolve_steer 进来。
    def approval_request(self, info: dict):
        self.flush_buffer()
        cprint("⚠️ 危险操作待确认", Ansi.YELLOW + Ansi.BOLD, emit_func=self._emit)
        cprint(f"  {info.get('summary', '')}", Ansi.YELLOW, emit_func=self._emit)
        cprint("  输入 y 批准 / n 拒绝 / a 本会话不再询问", Ansi.DIM, emit_func=self._emit)

    def approval_result(self, info: dict, approved: bool, reason: str):
        verdict = "已批准" if approved else f"未批准（{reason}）"
        cprint(f"  [approval] {verdict}", Ansi.DIM, emit_func=self._emit)

    def clarify_request(self, info: dict):
        self.flush_buffer()
        cprint("❓ 需要澄清", Ansi.CYAN + Ansi.BOLD, emit_func=self._emit)
        cprint(f"  {info.get('question', '')}", Ansi.CYAN, emit_func=self._emit)
        for i, opt in enumerate(info.get("options") or [], start=1):
            cprint(f"  {i}. {opt}", Ansi.DIM, emit_func=self._emit)
        cprint("  直接输入你的回答", Ansi.DIM, emit_func=self._emit)

    def clarify_result(self, info: dict):
        if info.get("status") == "answered":
            cprint(f"  [clarify] 已收到：{info.get('answer') or ''}",
                   Ansi.DIM, emit_func=self._emit)
        else:
            cprint(f"  [clarify] 未收到回答（{info.get('status')}）",
                   Ansi.DIM, emit_func=self._emit)

    def callbacks(self) -> "TurnCallbacks":
        from core.agent.agent import TurnCallbacks
        return TurnCallbacks(
            stream_delta=self.stream_delta,
            tool_call_start=self.tool_start,
...
def run_turns_threaded(
    ...
    def worker():
        ...
        emitter = _CliEmitter(on_content=on_content, emit_func=emit_func)
        cbs = emitter.callbacks()
        while message_queue:
            current_msg = message_queue.pop(0)
            plan_turn = plan_mode
            plan_mode = False
            from core.agent.turn_runner import AgentTurnParams, AgentTurnRunner
            runner = AgentTurnRunner(cmd_ctx["shared_ctx"], AgentTurnParams(
                source=src, text=current_msg, plan=plan_turn), cbs,
                agent=agent)  # CLI's long-lived instance, built once at startup
            try:
                result = runner.run()
            ...
            # Approved plan → persist it and queue the full-surface execution
            # turn (the loop continues with the normal toolset restored).
            outcome = runner.plan_outcome()
            if outcome:
                cprint("... executing approved plan", Ansi.DIM, emit_func=emit_func)
                message_queue.append(outcome.exec_text)
        ...
        while True:
            ...
            if text.startswith("/"):
                ...
                if head == "/resume":
                    ...
                    if not text:
                        cprint("Usage: /plan <task> — read-only planning turn, "
                               "approval auto-starts execution", Ansi.DIM, emit_func=_emit)
                        continue
                    plan_next = True
                else:
                    resp = handle_command(text, cmd_ctx)
...
def _build_cli_agent(config, platform_adapter=None, cwd=None):
    """CLI startup assembly — the same factory serve/gateway use, called ONCE
    (the CLI agent is long-lived: request_tools expansion, todo memory and
    steer semantics are per-instance state that a REPL wants to keep).

    Mirrors SharedContext → bootstrap_process → create_agent in three steps;
    cwd defaults to the launch directory (the CLI conversation's root).
    Returns (ctx, agent): the ctx feeds per-turn AgentTurnRunners."""
    from core.context import SharedContext, bootstrap_process
...
def run_chat(args):
    ...
    shared_ctx, agent = _build_cli_agent(config)
    ...
    if query:
        ...
        def tool_start(name, args_summary, by=None, tool_call_id=None):
