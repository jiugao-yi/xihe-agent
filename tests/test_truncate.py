...
def test_truncate_drops_from_given_row_inclusive(tmp_path, monkeypatch):
    ...
    db = SessionDB()
    source = SessionSource(platform="serve", chat_id="c1", user_id="u", chat_type="dm")
    session_key = db.build_key(source)
    session_id = db.get_or_create_session(source)
    ...
    deleted = db.truncate_messages_from(session_key, session_id, rows[2]["id"])
    assert deleted == 2
...
def test_truncate_first_user_message_empties_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(session_mod, "_DB_PATH", tmp_path / "sessions.db")
    db = SessionDB()
    source = SessionSource(platform="serve", chat_id="c2", user_id="u", chat_type="dm")
    session_key = db.build_key(source)
    session_id = db.get_or_create_session(source)
    ...
    assert db.truncate_messages_from(session_key, session_id, first["id"]) == 2
    assert db.load_messages_with_id(session_id) == []
    # other sessions untouched
    other = SessionSource(platform="serve", chat_id="c3", user_id="u", chat_type="dm")
    other_id = db.get_or_create_session(other)
    db.append_message(other_id, "user", "keep")
    db.truncate_messages_from(session_key, session_id, first["id"])
