...
import core.support.approvals as _approvals
...
def test_terminal_killing_agent_host_process_asks():
    # taskkill /im xihe.exe 会杀掉宿主 serve 进程，turn 随进程静默消失
    for cmd in ('taskkill /f /im xihe.exe', 'Stop-Process -Name xihe -Force',
                'taskkill /pid 123 & taskkill /im XIHE.EXE'):
        decision, summary = evaluate("terminal", {"command": cmd}, MANUAL)
        assert decision == "ask", cmd
        assert "self-termination" in summary
    # 杀其他进程不受影响
    assert evaluate("terminal",
                    {"command": "taskkill /f /im notepad.exe"}, MANUAL)[0] == "allow"


def test_terminal_killing_own_pid_asks():
    # /PID 形式的自缢：命令里没有 "xihe" 字样，靠比对本进程 PID 识别
    for cmd in (f"taskkill /f /pid {os.getpid()}",
    ...
    # 杀别的 PID 不受影响
    assert evaluate("terminal",
                    {"command": f"taskkill /f /pid {os.getpid() + 1}"},
                    MANUAL)[0] == "allow"
...
def test_chat_approval_key_overrides_bucket(make_agent, monkeypatch):
    ...
    from core.agent import title_generator
