...
- 页面内容必须脱敏：不写入密钥 / token / 凭据 / 账号口令；内网地址用占位符（如 `<内部网关IP>`）替代
- 不编造知识库或明确外部来源不支持的知识
- 保持 `raw/` 仅追加
- 尽可能保持来源引用明确
- 标注矛盾和不确定性而非隐藏
...
- `status` 取值约定：`active`（默认，页面结论有效）/ `deprecated`（**整页**结论已被推翻或所述机制/子系统已删除，正文保留为历史参考；页首须有过时注记并指向后继或删除原因）/ `archived`（实体专用，归档不再维护）。**只有部分小节过时的页面保持 `active`**，用页首或节内 ⚠️ 订正注记区分——整页与局部靠 status 机器可分，不靠读正文猜
