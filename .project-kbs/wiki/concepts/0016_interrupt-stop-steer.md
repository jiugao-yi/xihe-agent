...
`core/support/interrupt.py` 用 `contextvars.ContextVar` 存"当前线程跑的 agent"。agent 分发循环在每个工具调用前后 `bind_current_agent(self)`/`reset`（顺序+并行两条路径都绑）。`is_interrupted()` 读**当前 agent** 的标志——**按会话隔离**：A 的 `/stop` 不会让 B 的工具误退（旧的全局 `threading.Event` 会串）。
