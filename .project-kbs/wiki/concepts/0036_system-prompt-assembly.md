...
系统提示词由 `core/agent/prompts.py` 的**声明式层表**装配：`build_system_prompt()` 拿一个 `PromptCtx` 顺序走 `LAYERS: list[Layer]`，每层返回该节文本或 `None` 跳过，非 None 按**表序**拼接，最后 `expand_agent_vars` 展开 `${AGENT_HOME}`。所有工具相关层 key off `ctx.tools` —— **指导层永不宣传实际不可调用的工具**。
