...
## 双引擎驱动架构（src/core/services/external_agent.py）
...
- 代码：`src/core/services/external_agent.py`（机制层 + 双 driver）、`src/tools/external_agent_tool.py`（`_ENGINES` 对称：bin 检查/凭证/permission_mode 默认/约定文件 AGENTS.md）、`src/core/agent/prompts.py`（EXTERNAL_AGENT_GUIDANCE 双引擎触发词）。config：`external_agents.claude|codex`（见 `config.example.yaml`；codex 另有 `base_url`（内联 provider，opt-in）/`wire_api`；`extra_args` 两引擎通用，原样追加在 resume 旗标前）。
