...
## 三值决策管线（evaluate，core/support/approvals.py:216）
...
模型发起 tool_call（terminal / ssh_exec / process / …）
  ...
  ▼
③ 拦截层 ToolRegistry.dispatch（唯一汇聚点，core/registry.py:272，handler 执行前）
  ├─ 无 parent_agent（cron no_agent 脚本 / 测试直调）→ 跳过门，直接执行
  └─ 有 parent_agent ↓
       ① 判定层 evaluate(name, args, config, session_key)      ← core/support/approvals.py:186
       │  优先级自上而下，命中即返：
