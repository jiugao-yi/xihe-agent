...
`tools/skills_tool.py`（`skills_list`/`skill_view`）/ `tools/skill_manager_tool.py`（`skill_manage` 6 action + `_atomic_write_text`）/ `core/agent/prompts.py`（`build_skills_prompt` / `clear_skills_cache` / `_scan_skills_index`）。
