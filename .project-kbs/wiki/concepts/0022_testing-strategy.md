...
| **L3 真 LLM + judge** | 任务是否达成 | LLM-as-judge + 结构断言 | 高/慢/花钱 | ⏳ 后续(复用 `core/agent/auxiliary_client.py` 当 judge,nightly 套件) |
| **L4 平台集成** | gateway 全链路 | 消息级断言 | 中 | ⏳ 后续(假平台 server + `BasePlatformAdapter`) |

**关键事实**:xihe 的 agent 循环很薄(`core/agent/agent.py` 的 `chat()`),逻辑大半在 tool handler 里。所以 L0+L1 覆盖 ~80% 回归面且完全确定、零成本——是地基。
...
xihe 的落地点(`core/agent/agent.py` `XiheAgent.__init__`):
