...
1. **UI 层**：`App.tsx:146-150` 按 activeConvId 查 map → activeWs（树/布局/运行面板的绑定判定）。绑定是派生值、刻意不存 ConvMeta（注释：survives syncConversations rebuilds——绑定只活在 map 里）。
2. **发送层**：`appStore.ts:1008-1016` —— 每次 `sendTurn` 都重新解析 `convWorkspace[convId] → workdir`，`stream.sendTurn(convId, text, workdir)`（`serveClient.ts:1012/1050-1055`）。**每轮一传，中途换绑定即时生效**，不是建会话时一次性带。
3. **serve 层**：`gateway/serve/chat.py:291-311` —— 从 turn 取 `cmd.cwd`，`is_dir()` 校验（不存在则丢弃 + warning），`create_agent(cwd=cwd)`。工具侧（terminal 的 `resolve_path`/`agent_base_dir`，`core/support/paths.py`）相对路径与子进程 cwd 落到该目录；未绑定会话 `cwd=None` → 回退 serve 进程 cwd。
...
1. **删工作空间**：`appStore.ts` removeWorkspace 清掉该空间所有绑定 → 之后这些对话不再带 cwd。
2. **workdir 在磁盘不存在/被移走**：serve `is_dir()` 不过 → 丢弃 cwd + warning → agent 回退 serve 进程 cwd。
3. **外层新建的对话**（未经工作空间）：本来就没绑定。

## CLI：完全独立的另一套

- **会话不可见**：CLI 会话列表（`-r` 选号、`/sessions`）只列 `platform="cli"`；serve 会话 key 是 `agent:main:serve:dm:{conv_id}`（[[0024]]），另一个命名空间。（订正 2026-09-14：workspaces.json 现还带 `windows` 键存窗口布局——与本页主题无关。）
