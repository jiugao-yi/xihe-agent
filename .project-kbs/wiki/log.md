...
- **回写**: 桌面端工作台批次沉淀（用户要求「把最近的主要改动整理下，看下哪些适合沉淀到wiki」）→ 新增三个变更页：[[0043_desktop-workbench-phase1]]（工作台 Phase 1 大特性）、[[0044_desktop-header-workspace-ux]]（头部交互规则 + inWorkbench 推导 + 品牌管线）、[[0045_desktop-git-gutter-marks]]（编辑器 git 行标记）。同步 wiki/index.md（三条）、active.md（顶部已完成条目 + **F1 条目订正为已交付**（三步早已落地）+ 知识库现状 41→44）。判定记录：banner 水印清理脚本本身不收录（一次性工具，管线已并入 0044）；minimap 说明不单独成页（属 Monaco 常识 + 0045 已提投影）。total_pages 41→44。
- **维护提醒**: 桌面端 main 进程 IPC 改动（git:headFile 等）需整体重启 `npm run dev`，Ctrl+R 只刷 renderer。

## 2026-09-01

- **回写**: 包结构重构沉淀（用户要求「wiki文档都按照新的更新了吧」）→ 新增变更页 [[0042_package-restructuring]](type=change)：三原则依据 + DAG 实测依赖方向 + 四步变更（serve 业务分模块 / `app/` 启动器 + platforms 归位 / registry 契约下沉 `core/registry.py` / core 三子包 + 基础设施对流）+ 兼容策略（git mv / 包 re-export / pyproject glob）+ 验证证据（pytest 466 绿、serve 冒烟 200）。**活页路径大同步**（重构后旧路径订正，均经代码核实）：concepts 17 页 + entities 1 页——`core/agent.py`→`core/agent/agent.py`、`core/prompts.py`/`core/compressor.py`/`core/auxiliary_client`/`core/prompt_context.py`→`core/agent/*`、`core/agent_defs.py`/`core/external_agent.py`→`core/services/*`、`tools/_approvals.py`→`core/support/approvals.py`（dispatch 门 `tools/__init__.py:248`→`core/registry.py:272`）、`tools/interrupt.py`→`core/support/interrupt.py`、`cli/app.py` 按语义分流（launcher 引用→`app/main.py`、SharedContext 归属→`core/context.py`）、`src/platforms/`→`src/gateway/platforms/`；[[0002]] 「Import chain 防循环」条目重写为「分层规则」（旧表述"tools/__init__ 不 import 任何 core 模块"已失实）；[[0011]] registry 归属改指 `core/registry.py`。**不动**：`raw/sources/`（仅追加）、`changes/00XX` 带日期历史（0039 affected_modules 等保留当时路径）、[[0017]] 已废弃页（历史存档）。active.md 顶部新增已完成条目 + 活引用修正（`core/services/external_agent.py`、`core/agent/agent.py:1008`）+ 知识库现状 40→41。全部被编辑页 `updated` 置 2026-09-01。total_pages 40→41。
- **维护提醒**: gateway/serve 长驻进程需重启切到新包结构；桌面端点「重启 xihe」（7788 serve 仍跑旧代码，此前为重装已结束过一次）。

## 2026-08-31

- **回写**: server tool 调研沉淀（用户明确要求「把 server tool 的调研结果沉淀到wiki，注意脱敏」）→ 新增概念页 [[0041_server-tool-support]](type=concept，设计参考/未实施)。内容：三家形态对照（Anthropic 版本化类型 / OpenAI Responses / 智谱 nested + inline 引用）+ 两种响应风格；**内部网关实测矩阵**（glm-5.2-zp 组 = litellm Anthropic 通道、`web_search_20250305` 真执行 + 信息性 tool_call `web_search_prime` + 合成 result 二轮验证；-openai 组 6 变体全部静默空转 + 400 校验错误证明声明层认识）；xihe 代码缺口表（agent.py 两处序列化/dispatch/repair/compressor/session 列/config 两循环/serve 流式协议，均 file:line）；方案设计（`server_tools` 透传非 ToolRegistry 成员 + 不 dispatch 直合成 result 保全部不变式 + 流式零协议改动 + 边界与 L2 测试）；两车道方向判断；开放问题（模型组决策/公网出网边界/流式未测/计费护栏）。**脱敏处理**：网关 IP 用 `<内部网关IP>` 占位，无任何密钥/凭据。探针脚本：OS temp `probe_server_tool*.py` / `probe_opengroup_*.py`。同步 wiki/index.md（0041 条目）、recent.md、active.md（知识库现状 + 后续方向待办 + thought_delta 待办补充非流式 thinking_blocks 观察）。
- **规则新增**: PROTOCOL.md 依据规则加**脱敏条目**（用户要求「录一条规则：wiki 里面的内容要脱敏」）——页面内容必须脱敏：不写入密钥/token/凭据/账号口令，内网地址用占位符（如 `<内部网关IP>`）替代。
- **盘存校正**: total_pages 实数核对——此前 lint-status/active 记 40，磁盘实数 39（concepts 26 非 27，+1 漂移）；本次新增 0041 后磁盘实数 40，与登记口径对齐（index.md 39→40）。
