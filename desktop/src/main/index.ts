...
// One window's persisted view state (mirrors the renderer's WindowState in
// lib/desktop.ts — main doesn't import renderer modules). Validated loosely on
// load; the renderer re-validates before use.
...
/** Normalize one tabsByWorkspace value: keep string→string[] entries. */
function normTabsByWs(v: unknown): Record<string, { tabs: string[]; active: string | null }> {
  const out: Record<string, { tabs: string[]; active: string | null }> = {}
...
//   ~/.xihe-desktop/settings.json  { "theme", "layoutMode" }
...
type LayoutMode = 'chat' | 'workbench'
const THEME_MODES: readonly string[] = ['dark', 'light', 'system']
const LAYOUT_MODES: readonly string[] = ['chat', 'workbench']
let currentTheme: ThemeMode = 'dark'
let currentLayoutMode: LayoutMode = 'chat'

function settingsStorePath(): string {
  return join(workspaceStoreDir(), 'settings.json')
}

async function readSettings(): Promise<{ theme: ThemeMode; layoutMode: LayoutMode }> {
  try {
    const parsed = JSON.parse(
      await fs.readFile(settingsStorePath(), 'utf8')
    ) as { theme?: unknown; layoutMode?: unknown }
    const t = parsed?.theme
    const l = parsed?.layoutMode
    return {
      theme: typeof t === 'string' && THEME_MODES.includes(t) ? (t as ThemeMode) : 'dark',
      layoutMode: typeof l === 'string' && LAYOUT_MODES.includes(l) ? (l as LayoutMode) : 'chat',
    }
  } catch {
    return { theme: 'dark', layoutMode: 'chat' }
  }
}
...
async function writeSettings(
  ...
  try {
    ...
    await fs.writeFile(tmp, JSON.stringify({ theme, layoutMode }, null, 2), 'utf8')
...
function registerIpc(): void {
  ...
  // System toast from the renderer (cron-result pushes). Main-process
  // Notification + AppUserModelId is the reliable path on Windows — the
  // renderer's web Notification silently fails in dev/unpackaged builds.
  ipcMain.handle('desktop:notify', (_e, title: string, body: string) => {
    if (!Notification.isSupported()) return false
    new Notification({ title: String(title).slice(0, 60), body: String(body).slice(0, 200) }).show()
    return true
  })
  ...
  ipcMain.handle('workspace:load', async () => {
    try {
      ...
      const ws = (parsed as { windows?: unknown }).windows
      if (Array.isArray(ws)) {
        for (const w of ws) {
          const rec = w as Partial<WindowState> | null
          if (!rec || typeof rec !== 'object' || typeof rec.id !== 'string') continue
          windows.push({
            id: rec.id,
            workspaceId: typeof rec.workspaceId === 'string' ? rec.workspaceId : null,
            convId: typeof rec.convId === 'string' ? rec.convId : null,
            tabsByWorkspace: normTabsByWs(rec.tabsByWorkspace),
          })
        }
      }
  ...
  ipcMain.handle('app:setTheme', async (_e, theme: unknown) => {
    ...
    const persisted = await writeSettings(currentTheme, currentLayoutMode)
    pushAppearance()
    return { ok: true, theme: currentTheme, persisted }
  })

  // Layout mode (chat vs workbench) is a pure renderer concern — persist only.
  ipcMain.handle('app:setLayoutMode', async (_e, mode: unknown) => {
    if (typeof mode !== 'string' || !LAYOUT_MODES.includes(mode)) {
      return { ok: false }
    }
    currentLayoutMode = mode as LayoutMode
    const persisted = await writeSettings(currentTheme, currentLayoutMode)
    return { ok: true, layoutMode: currentLayoutMode, persisted }
  })

  // File-tree git decorations. Event-driven from the renderer (fsVersion /
  // manual refresh) — never polled; git.ts enforces the timeout.
  ipcMain.handle('git:status', async (_e, workdir: unknown) => {
    if (typeof workdir !== 'string') return { ok: false, reason: 'error' }
    return gitStatus(workdir)
  })

  // HEAD version of one file — the base for the editor's gutter change marks.
  ipcMain.handle('git:headFile', async (_e, path: unknown) => {
    if (typeof path !== 'string' || !isAbsolute(path)) return { ok: false, reason: 'error' }
...
function createWindow(): void {
  // Dev: app path = desktop/ project root. Absent in stripped-down layouts —
  // guard so the window still opens without an icon.
