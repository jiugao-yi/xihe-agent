...
// TCP slot can otherwise hang ~20s — AbortController caps it; 5s also covers
// the serve-side SQLite busy_timeout=5s worst case).
const LIVENESS_INTERVAL_MS = 6_000
const LIVENESS_FETCH_MS = 5_000
// One missed probe ≠ dead: serve's single event loop is routinely blocked a
// few seconds by sync handlers (SQLite writes, disk scans, CDP probe) while a
// turn is running. Only report stopped after N consecutive misses (~18s); a
// real process exit is reported by the child 'exit' handler within seconds.
...
export class ServeSupervisor {
  ...
  private async livenessTick(): Promise<void> {
    ...
    const judged = this.livenessFailures >= LIVENESS_FAIL_THRESHOLD
    if (this.owned) {
      ...
      if (judged && this.child && !this.child.killed && this.status.state !== 'stopped') {
        this.emit('stopped', {
          message: `xihe连续${this.livenessFailures}次未响应 /health（约${Math.round(
            (this.livenessFailures * LIVENESS_INTERVAL_MS) / 1000,
          )}s），等待恢复…`,
          version: null,
        })
      }
