...
xihe的桌面控制面（control plane）。桌面 main 进程**内置托管 `xihe serve` 子进程**（启动 / 健康检查 / 崩溃重启 / 退出清理）——用户无需手敲 `xihe serve`、也见不到 "serve" 字样；渲染进程经 REST + WS 与该子进程对话。**xihe是唯一的内置 agent**，其它引擎（claude / codex）作为xihe的 `external_agent` 工具经由xihe访问，不作为并列进程。
...
- **内置xihe agent**：main spawn 并监管 `xihe` 进程，sidebar 显示其连接状态（运行中 / 启动中 / 未运行）；`/health` 就绪后连接 serve，未就绪时发送会就地报错提示（无 mock 回落）。
...
- **内置xihe**：xihe不是"经协议连接的远端服务"，而是 main 托管的子进程；桌面只有这一个 agent，不做并列/多实例。其它引擎走xihe的内部能力（claude/codex = `external_agent` 工具），不作为并列 connector。
- **能力上报**：serve 在 `/health` 与 WS `hello` 上报 model 与 capability descriptor；桌面当前展示模型名，不按 capability 分支 UI。
